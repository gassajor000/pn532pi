...
from PN532.pn532 import pn532, PN532_COMMAND_TGINITASTARGET, DMSG
...
class EmulateTag:
  ...
  def emulate(self, tgInitAsTargetTimeout: int = 0) -> bool:
    ...
    cc_size = len(compatibility_container)
    runLoop = True
    ...
    while (runLoop):
      ...
      if rx_data[C_APDU_INS] == ISO7816_SELECT_FILE:
        if p1 == C_APDU_P1_SELECT_BY_ID:
          ...
          elif (lc == 2 and rx_data[C_APDU_DATA] == 0xE1 and (rx_data[C_APDU_DATA + 1] == 0x03 or rx_data[C_APDU_DATA + 1] == 0x04)):
            out_buf = self.setResponse(RESPCMD_COMMAND_COMPLETE)
          ...
          if (ndef_tag_application_name_v2 == rx_data[C_APDU_P2: C_APDU_P2 + len(ndef_tag_application_name_v2)]):
            out_buf = self.setResponse(RESPCMD_COMMAND_COMPLETE)
          else:
        ...
        if currentFile == TAGFILE_NONE:
          ...
          if (p1p2_length > NDEF_MAX_LENGTH):
            ...
            compatibility_container = compatibility_container[:p1p2_length] + rx_data[:lc]
            self.setResponse(RESPCMD_COMMAND_COMPLETE)
        elif currentFile == TAGFILE_NDEF:
          if (p1p2_length > NDEF_MAX_LENGTH):
            out_buf = self.setResponse(RESPCMD_END_OF_FILE_BEFORE_REACHED_LE_BYTES)
          else:
            self.ndef_file = self.ndef_file[:p1p2_length] + rx_data[:lc]
            self.setResponse(RESPCMD_COMMAND_COMPLETE)
